...
PointPillarsDetection::PointPillarsDetection()
    : x_min_(apollo::perception::lidar::Params::kMinXRange),
      x_max_(apollo::perception::lidar::Params::kMaxXRange),
      y_min_(apollo::perception::lidar::Params::kMinYRange),
      y_max_(apollo::perception::lidar::Params::kMaxYRange),
      z_min_(apollo::perception::lidar::Params::kMinZRange),
      z_max_(apollo::perception::lidar::Params::kMaxZRange) {
...
bool PointPillarsDetection::Init(const DetectionInitOptions& options) {
  point_pillars_ptr_.reset(new apollo::perception::lidar::PointPillars(
...
bool PointPillarsDetection::Detect(const DetectionOptions& options, 
  ...
  cur_cloud_ptr_ = std::shared_ptr<base::PointFCloud>(
      new base::PointFCloud(*original_cloud));
  ...
  Eigen::Affine3d pose = translation * quater.toRotationMatrix();
  GetObjects(&segmented_objects, pose,
...
void PointPillarsDetection::GetObjects(
  ...
  for (int i = 0; i < num_objects; ++i) {
    ...
    // size
    object->center = Eigen::Vector3d(static_cast<double>(x),
                                     static_cast<double>(y),
                                     static_cast<double>(z));
    object->size = Eigen::Vector3f(dx, dy, dz);
