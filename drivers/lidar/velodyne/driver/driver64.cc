...
void Velodyne64Driver::Init() {
  ...
  //modify for bug WSBM-69
  if(config_.mode() == STRONGEST || config_.mode() == LAST) {
  	config_.set_npackets(static_cast<int>(ceil(packet_rate_ / frequency)));
  }else{ //DUAL
  	config_.set_npackets(static_cast<int>(ceil(packet_rate_ *2 / frequency)));
  }
...
bool Velodyne64Driver::Poll(const std::shared_ptr<VelodyneScan>& scan) {
  ...
  scan->mutable_header()->set_timestamp_sec(apollo::cyber::Time().Now().ToSecond());
...
bool Velodyne64Driver::CheckAngle(const VelodynePacket& packet) {
  ...
  for (int i = 0; i < BLOCKS_PER_PACKET_DRIVER; ++i) {    
    uint16_t angle = static_cast<uint16_t>(raw_ptr[i * BLOCK_SIZE_DRIVER + 3] * 256 +
                                           raw_ptr[i * BLOCK_SIZE_DRIVER + 2]);
