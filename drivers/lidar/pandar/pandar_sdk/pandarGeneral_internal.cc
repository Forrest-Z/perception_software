...
#include "lidar/pandar/pandar_sdk/input.h"
#include "lidar/pandar/pandar_sdk/pandarGeneral_internal.h"
...
PandarGeneral_Internal::PandarGeneral_Internal(
  ...
  for (uint16_t rotIndex = 0; rotIndex < ROTATION_MAX_UNITS; ++rotIndex) {
    float rotation = static_cast<float>(degreeToRadian(0.01f * static_cast<float>(rotIndex)));
  ...
  for (int i = 0; i < HS_LIDAR_L20_BLOCK_NUMBER; i++) {
    int temp_index = HS_LIDAR_L20_BLOCK_NUMBER - 1 - i;
    int temp_index_half = temp_index / 2;
    block20OffsetSingle_[i] = 28.58f + static_cast<float>(temp_index) * 55.56f;
    block20OffsetDual_[i] = 28.58f + static_cast<float>(temp_index_half) * 55.56f;
  }
...
int PandarGeneral_Internal::LoadCorrectionFile(std::string correction_content) {
  ...
  for (int i = 0; i < HS_LIDAR_L64_UNIT_NUM; ++i) {
    ...
    General_elev_angle_map_[i] = static_cast<float>(elev_angle[i]);
    General_horizatal_azimuth_offset_map_[i] = static_cast<float>(azimuthOffset[i]);
  }
...
int PandarGeneral_Internal::ParseRawData(Pandar40PPacket *packet,
  ...
  for (int i = 0; i < BLOCKS_PER_PACKET; i++) {
    ...
    block.sob = static_cast<unsigned short>((buf[index] & 0xff) | ((buf[index + 1] & 0xff) << 8));
    block.azimuth = static_cast<unsigned short>((buf[index + 2] & 0xff) | ((buf[index + 3] & 0xff) << 8));
...
int PandarGeneral_Internal::ParseL64Data(HS_LIDAR_L64_Packet *packet,
  ...
  packet->header.sob = static_cast<unsigned short>((recvbuf[index] & 0xff) << 8| ((recvbuf[index+1] & 0xff)));
  ...
  for(block = 0; block < packet->header.chBlockNumber; block++) {
    packet->blocks[block].azimuth = static_cast<unsigned short>((recvbuf[index] & 0xff) | \
...
int PandarGeneral_Internal::ParseL20Data(HS_LIDAR_L20_Packet *packet, \
  ...
  int block = 0;
  //Parse 8 Bytes Header
  packet->header.sob = static_cast<unsigned short>((recvbuf[index] & 0xff) << 8| \
...
int PandarGeneral_Internal::ParseGPS(PandarGPS *packet, const uint8_t *recvbuf, \
  ...
  int index = 0;
  packet->flag = static_cast<unsigned short>((recvbuf[index] & 0xff) | ((recvbuf[index + 1] & 0xff) << 8));
  index += GPS_PACKET_FLAG_SIZE;
  packet->year =
      static_cast<unsigned short>((recvbuf[index] & 0xff - 0x30) + (recvbuf[index + 1] & 0xff - 0x30) * 10);
...
void PandarGeneral_Internal::CalcPointXYZIT(Pandar40PPacket *pkt, int blockid,
  ...
  for (int i = 0; i < LASER_COUNT; ++i) {
    ...
    point.x = static_cast<float>(xyDistance * \
        ...
        (static_cast<float>(block->azimuth)) / 100.0f)));
    ...
    point.ring = static_cast<unsigned short>(i);
...
void PandarGeneral_Internal::CalcL64PointXYZIT(HS_LIDAR_L64_Packet *pkt, int blockid, \
  ...
  for (int i = 0; i < chLaserNumber; ++i) {
    ...
    point.intensity = static_cast<unsigned char>(unit.intensity);
