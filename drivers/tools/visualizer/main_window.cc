...
struct MainWindow::VideoImgProxy {
  ...
  union {
    ...
    CyberChannReader<drivers::CompressedImage>*
...
struct MainWindow::RadarData {
  ...
  CyberChannReader<drivers::ContiRadar>* channel_reader_;
  RadarData(void)
      : root_item_(),
        channel_name_item_(),
        action_item_(),
...
void MainWindow::openRadarChannel(bool b) {
  std::cout << "openRadarChannel" << std::endl;
...
void MainWindow::DoOpenRadarChannel(bool b, RadarData* radarProxy) {
  std::cout << "DoOpenRadarChannel" << std::endl;
  if (b) {
    ...
    if (!radarProxy->channel_reader_) {
      ...
      radarProxy->channel_reader_ =
          new CyberChannReader<drivers::ContiRadar>();
      ...
      std::cout << "start radarcallback" << std::endl;
      /*auto radarcallback =
          [this, radarProxy](
      ...
      auto radarcallback =
          [this, radarProxy](
              const std::shared_ptr<drivers::ContiRadar>& pdata) {
            this->RadarRenderCallback(pdata, radarProxy);
          };
      std::cout << "end radarcallback" << std::endl;
...
*/
void MainWindow::RadarRenderCallback(
    const std::shared_ptr<const drivers::ContiRadar>& rawData,
    RadarData* radar) {
  radar->reader_mutex_.lock();
  radar->reader_mutex_.unlock();
...
void MainWindow::PointCloudReaderCallback(
    const std::shared_ptr<const drivers::PointCloud>& pdata) {
  pointcloud_reader_mutex_.lock();
  pointcloud_reader_mutex_.unlock();
  ...
  if (b) {
    ...
    if (!pointcloud_channel_Reader_) {
      pointcloud_channel_Reader_ =
          new CyberChannReader<drivers::PointCloud>();
      ...
      auto pointCallback =
          [this](const std::shared_ptr<drivers::PointCloud>& pdata) {
...
void MainWindow::ImageReaderCallback(
    const std::shared_ptr<const drivers::Image>& imgData,
...
void MainWindow::ImageReaderCallback(
    const std::shared_ptr<const drivers::CompressedImage>& imgData,
    VideoImgProxy* theVideoImgProxy) {
  theVideoImgProxy->reader_mutex_.lock();
  if (theVideoImgProxy->dynamic_texture_ != nullptr && imgData != nullptr) {
    QImage img;
    ...
    if (!theVideoImg->image_reader_) {
      if (theVideoImg->isCompressedImage_) {
        theVideoImg->compressed_image_reader_ =
            new CyberChannReader<drivers::CompressedImage>();
      } else {
        theVideoImg->image_reader_ =
            new CyberChannReader<drivers::Image>();
      }
      ...
      if (theVideoImg->isCompressedImage_) {
        auto videoCallback =
            [this, theVideoImg](
                const std::shared_ptr<drivers::CompressedImage>&
        ...
        auto videoCallback =
            [this, theVideoImg](
                const std::shared_ptr<drivers::Image>& pdata) {
