...
bool RadarPoints::FillData(
  ...
  set_vertex_count(rawData->contiobs_size());
  buffer_ = new GLfloat[vertex_count() * vertex_element_count()];

  if (buffer_) {
    GLfloat* ptr = buffer_;
    for (int i =0; i < rawData->contiobs_size(); i ++) {
      ptr[0] = static_cast<float>(rawData->contiobs(i).longitude_dist());
      ptr[1] = static_cast<float>(rawData->contiobs(i).lateral_dist());
      ptr[2] = std::pow(10.0f, static_cast<float>(rawData->contiobs(i).rcs() / 20.0));
      ptr += vertex_element_count();
    }  // end for
