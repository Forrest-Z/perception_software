...
bool CameraComponent::Init() {
  camera_config_ = std::make_shared<CameraConfig>();
...
#else
raw_image_->image =
      reinterpret_cast<char*>(aligned_alloc(128,(raw_image_->image_size)*sizeof(char)));
#endif
...
void CameraComponent::run() {
  ...
  while (!apollo::cyber::IsShutdown()) {
    ...
    if(image_time.ToNanosecond() < last_time) {
      AERROR << "[dpp], wrong_time, frame_id:" << pb_image->header().frame_id() 
            << ", cur_time:" << image_time.ToNanosecond()
            << ", last__time:" << last_time;
    }
    last_time = image_time.ToNanosecond();
